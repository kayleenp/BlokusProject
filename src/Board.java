	    	...
	    	if(evt.getSource() == grid[row][col])
	    	{
	    		...
	    		for(int i=0; i<5; i++)
				{
					for (int j=0; j<5; j++)
					{
						...
						{	// check kesalahan
							if(boardValueInteger[row+i-2][col+j-2]>=1) {
	    		...
	    		for(int i=0; i<5; i++)
				{
					...
					{		
								if(pieceIntegers[i][j]==1) {
									...
									if(playerValue==1) {
									if(boardValueInteger[row+i-1][col+j-1]==0||boardValueInteger[row+i-1][col+j-1]<-1) {
										grid[row+i-1][col+j-1].setBackground(Color.cyan);
									}
									if(boardValueInteger[row+i-3][col+j-1]==0 || boardValueInteger[row+i-3][col+j-1]<-1) {
										grid[row+i-3][col+j-1].setBackground(Color.cyan);
									}
									if(boardValueInteger[row+i-1][col+j-3]==0 || boardValueInteger[row+i-1][col+j-3]<-1) {
										grid[row+i-1][col+j-3].setBackground(Color.cyan);
									}
									if(boardValueInteger[row+i-3][col+j-3]==0 || boardValueInteger[row+i-3][col+j-3]<-1) {
										grid[row+i-3][col+j-3].setBackground(Color.cyan);
									} }
									else if(playerValue==2) {
										if(boardValueInteger[row+i-1][col+j-1]==0 || boardValueInteger[row+i-1][col+j-1]<=-3 || boardValueInteger[row+i-1][col+j-1]==-1) {
											grid[row+i-1][col+j-1].setBackground(Color.pink);
										}
										if(boardValueInteger[row+i-3][col+j-1]==0 || boardValueInteger[row+i-3][col+j-1]<=-3 || boardValueInteger[row+i-3][col+j-1]==-1) {
											grid[row+i-3][col+j-1].setBackground(Color.pink);
										}
										if(boardValueInteger[row+i-1][col+j-3]==0 || boardValueInteger[row+i-1][col+j-3]<=-3 || boardValueInteger[row+i-1][col+j-3]==-1) {
											grid[row+i-1][col+j-3].setBackground(Color.pink);
										}
										if(boardValueInteger[row+i-3][col+j-3]==0 || boardValueInteger[row+i-3][col+j-3]<=-3 || boardValueInteger[row+i-3][col+j-3]==-1) {
									...
									if(boardValueInteger[row+i-3][col+j-2]<=0 && boardValueInteger[row+i-3][col+j-2]!=-playerValue) {
										boardValueInteger[row+i-3][col+j-2]=-playerValue;
									}
									if(boardValueInteger[row+i-2][col+j-3]<=0 && boardValueInteger[row+i-2][col+j-3]!= -playerValue) {
										boardValueInteger[row+i-2][col+j-3]=-playerValue;
									}
									if(boardValueInteger[row+i-2][col+j-1]<=0 && boardValueInteger[row+i-2][col+j-1]!= -playerValue) {
										boardValueInteger[row+i-2][col+j-1]=-playerValue;
									}
									if(boardValueInteger[row+i-1][col+j-2]<=0 && boardValueInteger[row+i-1][col+j-2]!= -playerValue) {
								...
								if(pieceIntegers[i][j]==-1) {
									...
									if(boardValueInteger[row+i-2][col+j-2]!=2 || boardValueInteger[row+i-2][col+j-2]<=-2) {
										boardValueInteger[row+i-2][col+j-2]= -playerValue;
										grid[row+i-2][col+j-2].setBackground(Color.white);
									}
									}
